...
def _find_amount_near(text: str, start: int, end: int, radius: int = 120):
...
# --- helper local para € (evita depender de fmt_eur global)
        ...
        if row is None or (COL_TEXTO is None) or (COL_TEXTO not in df.columns):
            ...
            for m in REF_LINE_RE.finditer(texto_flat):
                ...
                rows_sin_bbdd.append({
                    ...
                    "%Coincidencia": "—",
                    "ImporteFactura": (
                        f"{imp_val:,.2f}".replace(",", "X").replace(".", ",").replace("X", ".")
                    ) if imp_val is not None else "",
            ...
            # ---- Imputar MODA del importe por referencia (solo EN_BBDD; no-cero)
            if aplicar_moda and not df_encontradas.empty:
                # Normaliza ImporteNum a float (""/None/"nan" -> NaN)
                df_encontradas["ImporteNum"] = pd.to_numeric(df_encontradas["ImporteNum"], errors="coerce")

                def _mode_nonzero(series: pd.Series):
                ...
                # Moda por referencia del texto extraído
                moda_map = (
                    df_encontradas
                    .groupby("RefTextoExtraido")["ImporteNum"]
                    .apply(_mode_nonzero)
                    .to_dict()
                )
                ...
                # Rellena NaN/None/0 con la moda y sincroniza ImporteFactura
                def _fill(row: pd.Series) -> pd.Series:
                    val = row.get("ImporteNum")
                    if pd.isna(val) or val == 0:
                        m = moda_map.get(row["RefTextoExtraido"])
                        if m is not None:
                            row["ImporteNum"] = m
                            row["ImporteFactura"] = _fmt_num_local(m)
                    # Si hay número pero ImporteFactura está vacío, formatea
                    if (
                        (not isinstance(row.get("ImporteFactura", ""), str)
                         or not str(row["ImporteFactura"]).strip())
                        and pd.notna(row.get("ImporteNum"))
                    ):
                        row["ImporteFactura"] = _fmt_num_local(row["ImporteNum"])
                    return row

                df_encontradas = df_encontradas.apply(_fill, axis=1)
